...
class Instruction
  ...
  # keep in mind default of zero for parameter_modes!!!
end
...
class Computer
  ...
  def initialize(program)
    self.memory = program.dup
  end

  def parse_instruction(instruction_ptr)
  ...
  def execute_instruction(instruction)
    ...
    when :add
      ...
      #puts "Adding: #{operand1} + #{operand2}, store at #{output_address}"
  ...
  def execute_program
    instruction_ptr = 0
    loop do
      instruction = parse_instruction(instruction_ptr)
      break if instruction.command == :halt

      # Execute the command
      # puts "Instruction:"
      # puts "  opcode: #{instruction.opcode}"
      # puts "  parameter_modes: #{instruction.parameter_modes}"
      # puts "  parameters: #{instruction.parameters}"
      # puts "  command: #{instruction.command}"
      # puts "  length: #{instruction.length}"
      execute_instruction(instruction)

      instruction_ptr += instruction.length
    end
    # memory[0]
  end
end

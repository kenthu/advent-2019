...
class IntersectionFinder
  attr_accessor :wire1_path, :wire1_points
  attr_accessor :wire2_path, :wire2_points
  ...
  def distance_of_closest_intersection()
    self.wire1_points = self.class.walk_path(wire1_path)
    self.wire2_points = self.class.walk_path(wire2_path)

    intersection_points = (wire1_points & wire2_points).delete([0, 0])
    closest_intersection_point = intersection_points.min_by { |point| self.class.manhattan_distance(point) }
    self.class.manhattan_distance(closest_intersection_point)
  end
...
TEST_PATH_PAIRS.each do |test_case|
  ...
  distance_of_closest_intersection = finder.distance_of_closest_intersection
...
puts finder.distance_of_closest_intersection